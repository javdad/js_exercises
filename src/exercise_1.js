...
function formatPhoneNumber(phoneNumber) {
	if (!phoneNumber) {
		throw new Error(
			"Phone number must be either 9 or 12 characters long"
		)
	}
	const numStr = phoneNumber.toString()
	if (!numStr || (numStr.length != 9 && numStr.length != 12)) {
		throw new Error(
			"Phone number must be either 9 or 12 characters long"
		)
	}
	if (numStr.length === 9) {
		return `+998 ${numStr.slice(0, 2)} ${numStr.slice(
			2,
			5
		)} ${numStr.slice(5, 7)} ${numStr.slice(7)}`
	}
	return `+${numStr.slice(0, 3)} ${numStr.slice(3, 5)} ${numStr.slice(
		5,
		8
	)} ${numStr.slice(8, 10)} ${numStr.slice(10)}`
}
